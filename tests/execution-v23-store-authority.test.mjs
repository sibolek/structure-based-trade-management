...
test("V2.3 transaction starts from latest durable store and cannot erase newer V2.4 state", () => {
  const storage = memoryStorage();
  ...
  const committedProjection = transactV23ExecutionProjection({
    storage,
...
test("V2.3 projection transaction preserves unknown forward-compatible namespaces", () => {
  const storage = memoryStorage();
  transactExecutionBoardStore({ storage, mutate: (store) => ({
    ...store,

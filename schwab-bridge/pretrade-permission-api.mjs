...
function routeFromPath(pathname) {
  const match = pathname.match(/^\/api\/candidates\/([^/]+)\/versions\/(\d+)\/permission\/evaluate$/);
...
function statusForError(error) {
  ...
  if (
    ...
    || code === "PERMISSION_OPERATION_ID_CONFLICT"
  ...
  if (
    ...
    || code === "CORRUPT_PERMISSION_ATTEMPT_REPOSITORY"
    || code === "EACCES"
    || code === "ENOSPC"
    || code === "EROFS"
    || code === "EIO"
...
export function createPreTradePermissionApiHandler({
  permissionPipeline,
  lifecycleCoordinator,
  maxBodyBytes = DEFAULT_MAX_BODY_BYTES,
} = {}) {
  if (!permissionPipeline || typeof permissionPipeline.evaluate !== "function") {
    throw new Error("permissionPipeline is required");
  }
  ...
  return async function handlePreTradePermissionApi(req, res) {
    ...
    try {
      ...
      const result = await permissionPipeline.evaluate({
        ...payload,
      ...
      json(res, statusForError(error), {
        error: error.message,
        code: error.code || "PRETRADE_PERMISSION_API_ERROR",
